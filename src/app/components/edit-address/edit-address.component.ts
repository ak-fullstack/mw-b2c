...
export class EditAddressComponent {
  ...
  states: any[] = [];
   addressForm = new FormGroup({
    id: new FormControl('', [Validators.required]), // Assuming you have an addressId to edit
  ...
  ngOnInit(): void {
    ...
    this.addressForm.patchValue(this.address); // Patch the form with the address data
  ...
  onSubmit(){
    ...
    this.apiService.editCustomerAddress(payload).subscribe({
      next: (res: any) => {
        this.close();
      },
      error: (error: any) => {
    ...
    @Input() address:any = {};
    @Output() closeEditAddressEvent = new EventEmitter<void>();

  close() {
    this.closeEditAddressEvent.emit();
  }
